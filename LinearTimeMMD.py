"""
Linear Time Kernel Two-Sample Test Simulation
...
# Import functions from the accompanying functions file
...
# Experiment Runner
# =============================================================================
def run_linear_time_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep):
    def process_ss(ss):
        # 1. Generate Data
        if dgp_set == 0:
        ...
        _, p_val, _ = compute_linear_time_mmd(X, Y)
        return (p_val < 0.1, p_val < 0.05, p_val < 0.01)

    # Parallel processing to speed up Monte Carlo repetitions
...
print("Linear Time MMD Test Simulation")
print("=" * 60)
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_linear_time_experiment(dgp_number=dgp, mm=mm, nn=nn, dd=dd,
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_linear_time_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
