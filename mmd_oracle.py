...
# %%
def run_optimized_mmd_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep):
    def process_ss(ss):
        # 1. Generate Data
        if dgp_set == 0:
        ...
        # 2. Run Optimized MMD Test

        pvalue_ker = optimized_mmd_test(X, Y, n_permutations=500, seed=ss)
...
# %%
print("Optimized MMD Two-Sample Test Simulation (ICLR 2017)")
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_optimized_mmd_experiment(dgp_number=dgp, mm=mm, nn=nn, dd=dd, 
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_optimized_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd, 
