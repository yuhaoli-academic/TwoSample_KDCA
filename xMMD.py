...
# Import the xMMD test function
from functions_xmmd import xMMD_test
# %%
def run_xMMD_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep):
    """
    Runs the xMMD simulation for a specific configuration.
    ...
    def process_iteration(ss):
        ...
        # 2. Run xMMD Test (Return p-value)
        # We pass alpha=0.05 as default, but we only care about the returned p_val
        _, p_val = xMMD_test(X, Y, alpha=0.05)
...
# %%
print("xMMD Test Simulation (Permutation-Free)")
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_xMMD_experiment(dgp_number=dgp, mm=mm, nn=nn, dd=dd, 
...
for loc, scale in loc_scale_candidates:
    print(f"\\mu={scale}, \\sigma^2={scale}:")
    for dd in dd_candidates:
        r90, r95, r99 = run_xMMD_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd, 
