...
# Optional: change working directory (only needed if doing relative file I/O)
import os
os.chdir(dgp_path)
...
def truncate(dgp_number,mm,nn,dd,loc,scale,df, dgp_set, Nrep, Nb,j):
    def process_ss(ss):
        ...
        # Center K_XX
        C_X = np.eye(m) - np.ones((m, m)) / m
        K_centered_X = C_X @ K_XX_from_K_j @C_X
        
        
        # Center K_YY
        C_Y = np.eye(n) - np.ones((n, n)) / n
        K_centered_Y = C_Y @ K_YY_from_K_j @ C_Y
       

        # Center K_XY
        K_centered_XY = C_X @ K_XY_from_K_j @ C_Y
       

        mu_X = np.zeros(K_centered_X.shape[0])
        Sigma_X = np.eye(K_centered_X.shape[0])

        v_mat_X = multivariate_normal.rvs(mean=mu_X, cov=Sigma_X, size=Nb)

        mu_Y = np.zeros(K_centered_Y.shape[0])
        Sigma_Y =np.eye(K_centered_Y.shape[0])

        v_mat_Y = multivariate_normal.rvs(mean=mu_Y, cov=Sigma_Y, size=Nb)

        
        stat_kerb = (m+n)*((1/(m*(m-1)))*np.sum((v_mat_X.T * (K_centered_X @ v_mat_X.T)),axis=0) -(1/(m*(m-1)))* np.trace(K_centered_X)  + (1/(n*(n-1)))*np.sum((v_mat_Y.T * (K_centered_Y @ v_mat_Y.T)),axis=0) -(1/(n*(n-1)))* np.trace(K_centered_Y)  - (2/(m*n))*np.sum((v_mat_X.T * (K_centered_XY @ v_mat_Y.T)),axis=0))
...
nn_candidates = [50, 70, 100]
mm = 100
mm_nn_candidates = [(mm, nn) for nn in nn_candidates]
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        for mm, nn in mm_nn_candidates:
            print(f"d={dd}, m={mm}, n={nn}:", truncate(dgp_number=dgp,mm=mm, nn=nn, dd=dd, loc=0.0, scale=1.0, df=3, dgp_set=0, Nrep=1000, Nb=500, j=jj))
    print("\n")
#%%
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        for mm, nn in mm_nn_candidates:
            print(f"d={dd}, m={mm}, n={nn}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=loc, scale=scale, df=3, dgp_set=1, Nrep=1000, Nb=500, j=jj))
    print("\n")
...
for df in df_candidates:
    ...
    for dd in dd_candidates:
        for mm, nn in mm_nn_candidates:
            print(f"d={dd}, m={mm}, n={nn}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=0.0, scale=1.0, df=df, dgp_set=3, Nrep=1000, Nb=500, j=jj))
    print("\n") 
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        for mm, nn in mm_nn_candidates:
            print(f"d={dd}, m={mm}, n={nn}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=loc, scale=scale, df=3, dgp_set=4, Nrep=1000, Nb=500, j=jj))
    print("\n") 
