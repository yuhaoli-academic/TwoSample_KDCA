...
def truncate(dgp_number,mm,nn,dd,loc,scale,df, dgp_set, Nrep, Nb):
    def process_ss(ss):
        if dgp_set == 0:
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        print(f"dd={dd}:", truncate(dgp_number=dgp,mm=mm, nn=nn, dd=dd, loc=0.0, scale=1.0, df=3, dgp_set=0, Nrep=1000, Nb=500))
    print("\n")
#%%
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        print(f"dd={dd}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=loc, scale=scale, df=3, dgp_set=1, Nrep=1000, Nb=500))
    print("\n")
...
for df in df_candidates:
    print(f"df={df}:")
    for dd in dd_candidates:
        print(f"dd={dd}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=0.0, scale=1.0, df=df, dgp_set=3, Nrep=1000, Nb=500))
    print("\n") 
...
for loc, scale in loc_scale_candidates:
    print(f"loc={loc}, scale={scale}:")
    for dd in dd_candidates:
        print(f"dd={dd}:", truncate(dgp_number=1,mm=mm, nn=nn, dd=dd, loc=loc, scale=scale, df=3, dgp_set=4, Nrep=1000, Nb=500))
    print("\n") 
