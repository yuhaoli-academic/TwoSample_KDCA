...
# Import the converted MMD-FUSE function
from functions_MMDFUSE import mmdfuse
# %%
def run_mmdfuse_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep):
    """
    Runs the MMD-FUSE test simulation.
    """
    def process_iteration(ss):
        ...
        # 2. Run MMD-FUSE Test
        # Using default parameters from paper/code: Gaussian+Laplace, 10 bandwidths
        # Explicitly passing seed=None to rely on global entropy or randomness
        p_val = mmdfuse(X, Y,  seed=None)
        
        # 3. Check rejection at multiple levels for analysis
...
print("MMD-FUSE Test Simulation (NumPy Implementation)")
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        res = run_mmdfuse_experiment(dgp_number=dgp, mm=mm, nn=nn, dd=dd, 
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        res = run_mmdfuse_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd, 
