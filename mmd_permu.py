...
# Import the MMD test function
from functions_mmd import mmd_permutation_test


# %%
def run_mmd_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep, Nb):
    def process_ss(ss):
        # 1. Generate Data
        if dgp_set == 0:
        ...
        # 2. Run MMD Permutation Test
        pvalue_ker = mmd_permutation_test(X, Y, sigma, Nb=Nb)
...
# %%
print("MMD Permutation Test Simulation")
print("=" * 60)
...
for dgp in dgp_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=dgp, mm=mm, nn=nn, dd=dd,
                                           loc=0.0, scale=1.0, df=3, dgp_set=0, Nrep=Nrep, Nb=Nb)
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
                                           loc=loc, scale=scale, df=3, dgp_set=1, Nrep=Nrep, Nb=Nb)
        print(f"d={dd}: ({r90:.3f}, {r95:.3f}, {r99:.3f})")
    print("\n")
...
print("=" * 60)
print("Set 2: T-distribution Deviation")
print("=" * 60)
...
for df in df_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
                                           loc=0.0, scale=1.0, df=df, dgp_set=3, Nrep=Nrep, Nb=Nb)
        print(f"d={dd}: ({r90:.3f}, {r95:.3f}, {r99:.3f})")
    print("\n")
...
for loc, scale in loc_scale_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
                                           loc=loc, scale=scale, df=3, dgp_set=4, Nrep=Nrep, Nb=Nb)
        print(f"d={dd}: ({r90:.3f}, {r95:.3f}, {r99:.3f})")
    print("\n")
...
for scale in scale_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
                                           loc=0.0, scale=scale, df=3, dgp_set=1, Nrep=Nrep, Nb=Nb)
        print(f"d={dd}: ({r90:.3f}, {r95:.3f}, {r99:.3f})")
    print("\n")
...
for loc in location_candidates:
    ...
    for dd in dd_candidates:
        r90, r95, r99 = run_mmd_experiment(dgp_number=1, mm=mm, nn=nn, dd=dd,
                                           loc=loc, scale=1.0, df=3, dgp_set=1, Nrep=Nrep, Nb=Nb)
        print(f"d={dd}: ({r90:.3f}, {r95:.3f}, {r99:.3f})")
    print("\n")