...
from functions_bMMD import *

# %%
def run_mMMD_experiment(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep):
    """
    Wrapper function to run the B-test simulation experiment.
    It fits the calling signature used in the printing style template.
    """
    def process_ss(ss):
        # 1. Generate Data
        if dgp_set == 0:
        ...
        elif dgp_set == 2: # Usually set1_2 covers set 2 in provided code structure
            data_dgp = dgp_choose_set1_2(mm, nn, dd, loc, scale)
        elif dgp_set == 3:
        ...
        # 2. Median Heuristic for Bandwidth
        Z = np.vstack((X, Y))
        pairwise_dists = pdist(Z, 'euclidean')**2
        sigma = np.median(pairwise_dists)
        
        # 3. Compute p-value using B-test
        # Note: This function is named run_mMMD_experiment per your style, 
        # but performs the B-test as requested.
        pvalue_ker = b_test_p_value(X, Y, sigma)
...
# %%
print("B-test Simulation (Low Variance Kernel Test)")
