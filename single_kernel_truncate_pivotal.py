...
# Import optimized functions from functions.py
from functions import p_value_calculation
...
def truncate(dgp_number, mm, nn, dd, loc, scale, df, dgp_set, Nrep, Nb, j):
    """
    Run the test for a given DGP configuration and return rejection rates.
    Uses the optimized p_value_calculation from functions.py.
    """
    def process_ss(ss):
        ...
        # Compute p-value using the optimized function
        pvalue_ker = p_value_calculation(X, Y, j, sigma, p_hat, Nb)
